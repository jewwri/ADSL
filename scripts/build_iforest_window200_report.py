...
def build_iforest_report(iforest_root: Path, mcts_root: Path, output_root: Path, total_steps: int) -> list[Path]:
    ...
    report_lines = [
        ...
        "",
        "This report summarizes the completed `window_length=200`, `200,000`-step Isolation Forest sanitize-gated no-lookahead comparison ablation.",
        "",
        "## Sources",
        "",
        f"- Isolation Forest sanitize-gated source: `{iforest_root}`",
        f"- MCTS comparison source: `{mcts_root}`",
        "",
        "## Detector-Only Matrix Status",
        "",
        f"- completed Isolation Forest sanitize-gated logical runs: `{len(iforest_runs)}`",
    ...
    readme_path.write_text(
        "\n".join(
            [
                ...
                "",
                "This directory contains the completed Isolation Forest sanitize-gated no-lookahead analysis for the `window_length=200`, `200,000`-step study.",
                "",
                "## Included Artifacts",
                "",
                "- `iforest_window200_final_report.md`",
                ...
                "",
                "- sanitize-gated Isolation Forest runs are compared against the completed `window=200` MCTS clean and attack-none selected runs",
